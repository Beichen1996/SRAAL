...
#
def uncertainty_score(models, dataloaders, classes):
    models.eval()
    all_scores = []
    all_indices = []
    m = nn.Softmax(dim=1)
    with torch.no_grad():
        for inputs, labels, indices in dataloaders:
            inputs = inputs.cuda()
            labels = labels.cuda()
            indices = indices.cuda()
            predictions, _ = models(inputs)
            predictions = m(predictions)
            #calculate the uncertainty score
            maximum, _ = torch.max(predictions, 1)
            pp1 = 1/classes - (1-maximum)/(classes-1)
            pp1 = (classes-1) * torch.pow(pp1, 2)
            pp2 = 1/classes - maximum
            pp2 = torch.pow(pp2, 2)
            pp3 = (1/classes) * (pp1 + pp2)
            var = torch.var(predictions, 1)
            score = 1 - maximum*pp3/var

            all_scores.extend(score.cpu().numpy())
            all_indices.extend(indices.cpu().numpy())

    all_indices1 = np.asarray(all_indices)
    all_scores1 = np.asarray(all_scores)
    dic = {}
    for i in range(len(all_scores1)):
        dic[all_indices1[i]] = all_scores1[i]
    return dic #Store the uncertainty scores into the dic with indices
